...
SYSTEM_PROMPT = """你是一位知识整理专家。将音频转录文本整理为笔记。
...
内容部分自由组织，不需要固定章节标题，根据内容特点选择最合适的呈现方式。可以是连续段落、列表、表格的组合。"""
...
def run(args):
    ...
    if is_youtube_playlist(target_url):
        ...
        for idx, video in enumerate(videos, 1):
            ...
            try:
                ...
                if raw_text:
                    summary = summarize_with_llm(raw_text[:15000], video_title or title, llm_model, llm_base_url, llm_api_key)
                    if summary:
                ...
                for page_num in range(1, part_count + 1):
                    ...
                    try:
                        ...
                        if page_num <= len(pages):
                            ...
                            if raw_text:
                                summary = summarize_with_llm(raw_text[:15000], f"{video_title} - {part_name}", llm_model, llm_base_url, llm_api_key)
                                if summary:
    ...
    if raw_text:
        summary = summarize_with_llm(raw_text[:15000], video_title, llm_model, llm_base_url, llm_api_key)
        if summary:
            save_to_obsidian(summary, inbox)
